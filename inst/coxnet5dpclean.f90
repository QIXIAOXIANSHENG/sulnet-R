...
subroutine coxnet(parm,nobs,nvars,x,y,d,g,w,jd,vp,cl,ne,nx,nlam,flmin,ulam,thr,  maxit,isd,lmu,ca,ia,nin,dev0,dev,alm,nlp,jerr)
    implicit double precision(a - h, o - z)
    double precision x(nobs, nvars), y(nobs), d(nobs), g(nobs), w(nobs), vp(nvars), ulam(nlam)
    double precision ca(nx, nlam), dev(nlam), alm(nlam), cl(2, nvars)
...
subroutine cstandard(nobs, nvars, x, w, ju, isd, xs)
    implicit double precision(a - h, o - z)
    double precision x(nobs, nvars), w(nobs), xs(nvars)
    integer ju(nvars)
    do 10151 j = 1, nvars
...
subroutine coxnet1(parm, nobs, nvars, x, y, d, g, q, ju, vp, cl, ne, nx, nlam, flmin, &
  ulam, cthri, isd, maxit, lmu, ao, m, kin, dev0, dev, alm, nlp, jerr)
    implicit double precision(a - h, o - z)
    double precision x(nobs, nvars), y(nobs), q(nobs), d(nobs), g(nobs), vp(nvars), ulam(nlam &
      & )
...
subroutine outer(nobs, nk, d, dk, kp, jp, e, wr, w, jerr, u)
    implicit double precision(a - h, o - z)
    double precision d(nobs), dk(nk), wr(nobs), w(nobs)
    double precision e(nobs), u(nobs), b, c
...
subroutine vars(nobs, nvars, x, w, ixx, v)
    implicit double precision(a - h, o - z)
    double precision x(nobs, nvars), w(nobs), v(nvars)
    integer ixx(nvars)
    do 10951 j = 1, nvars
...
subroutine usk(nobs, nk, kp, jp, e, u)
    implicit double precision(a - h, o - z)
    double precision e(nobs), u(nk), h
    integer kp(nk), jp(nobs)
    h = 0.0
    do 10971 k = nk, 1, -1
        j2 = kp(k)
...
function risk(nobs, nvars, nk, d, dk, f, e, kp, jp, u)
    implicit double precision(a - h, o - z)
    double precision d(nobs), dk(nk), f(nobs)
    integer kp(nk), jp(nobs)
    double precision e(nobs), u(nk)
    nvars = nvars*1
    call usk(nobs, nk, kp, jp, e, u)
    u = log(u)
...
subroutine loglike(nobs, nvars, x, y, d, g, w, nlam, a, flog, jerr)
    implicit double precision(a - h, o - z)
    double precision x(nobs, nvars), y(nobs), d(nobs), g(nobs), w(nobs), a(nvars, nlam), flog(nlam)
...
11031       continue
            continue
    flog(lam) = risk(nobs, nvars, nk, dq, dk, f, e, kp, jp, uu)
11021   continue
    continue
10180  continue
