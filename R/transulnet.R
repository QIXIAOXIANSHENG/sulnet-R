...
#' @param loo Using LOO or not, default is \code{TRUE}.
...
transulnet <- function(d0, dA, method = c("suni_tsloo", "suni_sloo", "suni_s"), loo = TRUE,
                       ...) {
  ################################################################################
  ## target data setup
  ...
  ################################################################################
  ## source data setup
  pA <- unique(sapply(dA, function(x) {
    dim(x$x)[2]
  }))
