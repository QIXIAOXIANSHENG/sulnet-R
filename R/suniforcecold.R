...
suniforcecold <- function(x, y, nlam, flmin, ulam, isd, intr, eps, dfmax, pmax, jd, pf,
  ...
  if(negOnly){
    if(!is.null(alpha)){
      ...
      for(a in seq_along(alpha)){
        ...
        b0list[[paste0("alpha_", alpha[a])]] = b0temp
        betamat[[a]] <- do.call(cbind, betalist)
        dfmat[[a]] <- dftemp
        npassesmat[[a]] <- npassestemp
        jerrmat[[a]] <- jerrtemp
      ...
      for(a in seq_along(lamPos)){
        ...
        b0list[[paste0("lamPos_", lamPos[a])]] = b0temp
        betamat[[a]] <- do.call(cbind, betalist)
        dfmat[[a]] <- dftemp
        npassesmat[[a]] <- npassestemp
        jerrmat[[a]] <- jerrtemp
  ...
  if(!is.null(alpha)){
    ...
    for(a in seq_along(alpha)){
      ...
      fb0list[[paste0("alpha_", alpha[a])]] = b0temp
      betatemp = do.call(cbind, betalist)
      fbetamat[[a]] <- betatemp

      row_idx <- betatemp@i + 1
      col_ptrs <- betatemp@p
      col_idx <- rep(seq_along(col_ptrs[-1]), diff(col_ptrs))
      beta_result <- betatemp
      beta_result@x <- betatemp@x * unibeta[cbind(row_idx, col_idx)]

      betamat[[a]] = beta_result
      b0list[[paste0("alpha_", alpha[a])]] = b0temp + colSums(unibeta0 * betatemp)
    ...
    for(a in seq_along(lamPos)){
      ...
      fb0list[[paste0("lamPos_", lamPos[a])]] = b0temp
      betatemp = do.call(cbind, betalist)
      fbetamat[[a]] <- betatemp
      ...
      betamat[[a]] = beta_result
      b0list[[paste0("lamPos_", lamPos[a])]] = b0temp + colSums(unibeta0 * betatemp)
