...
#'
#' The function runs \code{\link{sulnet}} \code{nfolds}+1 times; the first to
...
#'
#' @param x \code{x} matrix as in \code{\link{sulnet}}.
...
#' beta <- rep(c(1, 0), each = c(5, 5))
#' y <- x %*% beta + rnorm(200, 0, 0.1)
#' # 1. solution paths for the LASSO penalized least squares.
#' # To use LASSO set lambda2 = 0.
#'
#' cv1 <- cv.sulnet(x = x, y = y, lambda2 = 0, method = "ls")
#' plot(cv1)
#'

#'
#' @export
cv.sulnet <- function(x, y, lambda = NULL, pred.loss = c("misclass", "loss"),
                      nfolds = 5, foldid, ...) {
  ...
  y <- drop(y)
  sulnet.object <- sulnet(x, y,
    lambda = lambda,
    ...
  )
  lambda <- sulnet.object$lambda
  ## predict -> coef
  nz <- sapply(coef(sulnet.object, type = "nonzero"), length)
  if (missing(foldid)) {
  ...
  fun <- paste("cv", class(sulnet.object)[[2]], sep = ".")
  ...
  cvname <- cvstuff$name
  out <- list(
    lambda = lambda, cvm = cvm, cvsd = cvsd,
    cvupper = cvm + cvsd, cvlo = cvm - cvsd,
    nzero = nz, name = cvname, sulnet.fit = sulnet.object
  )
