...
##' the stored \code{"sulnet.fit"} object, and the optimal value chosen for
##' \code{lambda}.
##'
...
##' data(FHT)
##' set.seed(2011)
##' cv=cv.sulnet(FHT$x, FHT$y, lambda2 = 1, pred.loss="misclass",
##'              lambda.factor=0.05, nfolds=5)
##' pre = predict(cv$sulnet.fit, newx = FHT$x, s = cv$lambda.1se,
##'               type = "class")
##'
##' @export
predict.cv.sulnet <- function(object, newx, s = c("lambda.1se", "lambda.min"),
                              ...) {
  if (is.numeric(s))
  ...
  predict(object$sulnet.fit, newx, s = lambda, ...)
}
