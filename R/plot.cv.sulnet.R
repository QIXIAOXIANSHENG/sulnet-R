...
#' A plot is produced.
#'
#' @param x fitted \code{\link{cv.sulnet}} object
...
#' cv <- cv.sulnet(x = x, y = y)
#' plot(cv)
#'
#' @importFrom graphics points abline
#' @export
plot.cv.sulnet <- function(x, sign.lambda = 1, ...) {
  cvobj <- x
  xlab <- "log(Lambda)"
  if (sign.lambda < 0) {
    xlab <- paste("-", xlab, sep = "")
  ...
  axis(
    side = 3, at = sign.lambda * log(cvobj$lambda), labels = paste(cvobj$nz),
    tick = FALSE, line = 0
  )
