...
sunicold <- function(x, y, nlam, flmin, ulam, isd, intr, eps, dfmax, pmax, jd, pf,
  ...
  if(negOnly){
    if(!is.null(alpha)){
      ...
      for(a in seq_along(alpha)){
        fit <- .Fortran("sunicoldstart", lam2, lamPos, nobs, nvars, x, as.double(y), jd, pf, pf2,
      ...
      for(a in seq_along(lamPos)){
        fit <- .Fortran("sunicoldstart", lam2,lamPos[a], nobs, nvars, x, as.double(y), jd, pf, pf2,
                        dfmax, pmax, nlam, flmin, ulam, eps, isd, intr, maxit,
                        nalam = integer(1), b0 = double(nlam),
                        beta = double(pmax * nlam), ibeta = integer(pmax),
                        nbeta = integer(nlam), alam = double(nlam),
  ...
  if(!is.null(alpha)){
    ...
    for(a in seq_along(alpha)){
      fit <- .Fortran("sunicoldstart", lam2, lamPos, nobs, nvars, f, as.double(y), jd, pf, pf2,
                      dfmax, pmax, nlam, flmin, ulam, eps, isd, intr, maxit,
                      nalam = integer(1), b0 = double(nlam),
                      beta = double(pmax * nlam), ibeta = integer(pmax),
                      nbeta = integer(nlam), alam = double(nlam),
    ...
    for(a in seq_along(lamPos)){
      fit <- .Fortran("sunicoldstart", lam2, lamPos[a], nobs, nvars, f, as.double(y), jd, pf, pf2,
                      dfmax, pmax, nlam, flmin, ulam, eps, isd, intr, maxit,
                      nalam = integer(1), b0 = double(nlam),
                      beta = double(pmax * nlam), ibeta = integer(pmax),
                      nbeta = integer(nlam), alam = double(nlam),
