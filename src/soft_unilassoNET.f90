...
! ---------------------------------------------------------------------------- !
SUBROUTINE soft_unilassoNET(lam2, lamPos, nobs, nvars, x, y, jd, pf, pf2, dfmax, pmax, nlam,&
    ...
    CALL soft_unilassoNETpath(lam2, lamPos, maj, nobs, nvars, x, y, ju, pf, pf2, dfmax, pmax,&
    ...
    RETURN
END SUBROUTINE soft_unilassoNET

! ---------------------------------------------------------------------------- !
SUBROUTINE soft_unilassoNETpath(lam2, lamPos, maj, nobs, nvars, x, y, ju, pf, pf2, dfmax,&
            ...
            DO
                ...
                DO k = 1, nvars
                    IF (ju(k) /= 0) THEN
                        ...
                        !  END IF
                        IF (u > v) THEN
                            b(k) = (u - v)/(maj(k) + pf2(k)*lam2)
                        ELSE IF (u < -v - lamPos) THEN
                            b(k) = (u + v + lamPos)/(maj(k) + pf2(k)*lam2)
                        ELSE
                            b(k) = 0.0D0
                        END IF
                        d = b(k) - oldb
    ...
    RETURN
END SUBROUTINE soft_unilassoNETpath
