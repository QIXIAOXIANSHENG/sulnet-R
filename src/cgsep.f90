...
! ---------------------------------------------------------------------------- !
SUBROUTINE cgsep(lam2, lamPos, nobs, nvars, x, y, jd, pf, pf2, dfmax, pmax, nlam,&
     & flmin, ulam, eps, isd, intr, maxit, nalam, b0, beta, ibeta, nbeta, alam,&
     & npass, jerr, alpha, iglamPos, cgw)
    ! -------------------------------------------------------------------------- !
    IMPLICIT NONE
    ! -------- INPUT VARIABLES -------- !
    INTEGER :: nobs
    ...
    CALL cgsepNETpath(lam2, lamPos, maj, nobs, nvars, x, y, ju, pf, pf2, dfmax, pmax,&
         & nlam, flmin, ulam, eps, maxit, nalam, b0, beta, ibeta, nbeta, alam,&
         & npass, jerr, intr, alpha, iglamPos, cgw)
    ...
    RETURN
END SUBROUTINE cgsep

! ---------------------------------------------------------------------------- !
SUBROUTINE cgsepNETpath(lam2, lamPos, maj, nobs, nvars, x, y, ju, pf, pf2, dfmax,&
            ...
            DO
                ...
                DO k = 1, nvars
                    IF (ju(k) /= 0) THEN
                        ...
                        !     b(k) = 0.0D0
                        !  END IF
                        lampostemp = (lampos - al) * cgw(k) + al
                        IF (u > vpos) THEN
                            b(k) = (u - vpos)/(maj(k) + pf2(k)*lam2)
                        ELSE IF (u < - lampostemp) THEN
                            b(k) = (u + lampostemp)/(maj(k) + pf2(k)*lam2)
                        ELSE
                            b(k) = 0.0D0
                        END IF
                        d = b(k) - oldb
